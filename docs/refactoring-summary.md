...
이 문서는 콘서트 티켓팅 서버의 리팩토링 의사결정을 설명하기 위한 요약이다. 단순히 파일을 옮긴 기록이 아니라, 대용량 트래픽과 동시성 문제가 있는 도메인에서 어떤 기준으로 구조와 API 계약을 개선했는지 정리한다.
...
- API 탐색과 테스트가 쉬워졌다.
- API 표면을 문서와 실행 환경에서 직접 확인할 수 있다.
